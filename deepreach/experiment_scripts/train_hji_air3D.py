...
p.add_argument('--logging_root', type=str, default='./hji_logs/logs_air3D', help='root for logging')
...
p.add_argument('--learn_symmetry', action='store_true', default=False, required=False, help='Whether to back-prop symmetric loss during training')
...
val_dataset = dataio.ReachabilityAir3DValidationSet(validation_data_path='./deepreach/validation_sets/air3D_validation_set.pkl')
dataloader = DataLoader(dataset, shuffle=True, batch_size=opt.batch_size, pin_memory=True, num_workers=0)
val_dataloader = DataLoader(val_dataset, shuffle=False, batch_size=2048 * 4, pin_memory=True)

model = modules.Air3D_SymmetryICNet(in_features=4, out_features=1, type=opt.model, mode=opt.mode,
                             final_layer_factor=1., hidden_features=opt.num_nl, num_hidden_layers=opt.num_hl,
                             omega_max=opt.omega_max, velocity=opt.velocity, angle_alpha=opt.angle_alpha, collisionR=opt.collisionR,
                             use_symmetry=opt.use_symmetry, provide_initial_condition=opt.provide_initial_condition)
model.cuda()

# Define the loss
loss_fn = loss_functions.initialize_hji_air3D(dataset, opt.minWith, opt.use_symmetry)

root_path = os.path.join(opt.logging_root, opt.experiment_name)

def val_fn(model, ckpt_dir, epoch, writer=None, validation_data=None):
...
training.train(model=model, train_dataloader=dataloader, val_dataloader=val_dataloader, epochs=opt.num_epochs, lr=opt.lr,
               ...
               use_lbfgs=opt.use_lbfgs, validation_fn=val_fn, start_epoch=opt.checkpoint_toload, training_setting=opt)
