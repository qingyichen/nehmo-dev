...
p.add_argument('--logging_root', type=str, default='./hji_logs/logs_PointPlane', help='root for logging')
...
p.add_argument('--velocity_limit', type=float, default=0.5, required=False, help='Max velocity of the robot in x or y direction')
...
val_dataset = dataio.ReachabilityPointPlaneSource(numpoints=30000, collisionR=opt.collisionR, velocity=opt.velocity_limit, 
                                          pretrain=False, tMin=opt.tMin,
                                          tMax=opt.tMax, counter_start=opt.counter_start, counter_end=opt.counter_end,
                                          pretrain_iters=opt.pretrain_iters, seed=100,
                                          num_src_samples=opt.num_src_samples, validation=True)
dataset = dataio.ReachabilityPointPlaneSource(numpoints=65000, collisionR=opt.collisionR, velocity=opt.velocity_limit, 
...
val_dataloader = DataLoader(val_dataset, shuffle=True, batch_size=opt.batch_size, pin_memory=True, num_workers=0)

model = modules.PointPlane_ICNet(in_features=dataset.num_states+1, out_features=1, type=opt.model, mode=opt.mode,
                             final_layer_factor=1., hidden_features=opt.num_nl, num_hidden_layers=opt.num_hl, collisionR=opt.collisionR, 
                             max_velocty=opt.velocity_limit, provide_initial_condition=opt.provide_initial_condition)
model.cuda()

# Define the loss
loss_fn = loss_functions.initialize_hji_pointPlane(dataset, opt.minWith, opt.use_symmetry)
