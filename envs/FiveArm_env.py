...
class FiveArmEnv:
    ...
    def __init__(self,
            ...
            # GOAL related
            goal_threshold = 0.03, # goal threshold
        ...
        self.continuous_joints = range(30)
        ...
        self.robot3_collision_manager = trimesh.collision.CollisionManager()
        self.robot4_collision_manager = trimesh.collision.CollisionManager()
        self.robot5_collision_manager = trimesh.collision.CollisionManager()
         ###
        ...
        for link in self.robot.links:
            ...
            if not self.collision_use_visual and link.collision_mesh is not None:
                ...
                if link.name[0] == '1':
                    ...
                    self.robot3_collision_manager.add_object(link.name, mesh=mesh)
                elif link.name[0] == '4':
                    self.robot4_collision_manager.add_object(link.name, mesh=mesh)
                elif link.name[0] == '5':
                    self.robot5_collision_manager.add_object(link.name, mesh=mesh)
    ...
    def _generate_free_configuration(self, n_tries=100):
        ...
        pos_lower_lim = np.array([-5, -1.0, 1.0, -0.2, -2.5, -0.0001] * 5)
        pos_upper_lim = np.array([-1.5, 0.5, 2.0, 0.8, -0.5, 0.0] * 5)
        qpos_to_env_mapping = np.array([0, 6, 12, 18, 24, 1, 13, 7, 19, 25, 26, 20, 14, 8, 2, 9, 15, 3, 27, 21, 28, 22, 4, 10, 16, 5, 11, 17, 23, 29], dtype=np.int32)
    ...
    def get_reward(self, action):
        ...
        dist = np.max(np.abs(self._wrap_cont_joints(self.qpos - self.qgoal)))
    ...
    def _different_arm_collision_check(self, fk_dict, distance=False):
        for name, transform in fk_dict.items():
            if name in self.robot_collision_objs:
                if name[0] == '1':
                    ...
                    self.robot3_collision_manager.set_transform(name, transform)
                elif name[0] == '4':
                    self.robot4_collision_manager.set_transform(name, transform)
                elif name[0] == '5':
                    self.robot5_collision_manager.set_transform(name, transform)
                elif  name != 'world_base':
                    raise RuntimeError
        if not distance:
            collision12, name_pairs = self.robot1_collision_manager.in_collision_other(self.robot2_collision_manager, return_names=True)
            collision13, name_pairs = self.robot1_collision_manager.in_collision_other(self.robot3_collision_manager, return_names=True)
            collision14, name_pairs = self.robot1_collision_manager.in_collision_other(self.robot4_collision_manager, return_names=True)
            collision15, name_pairs = self.robot1_collision_manager.in_collision_other(self.robot5_collision_manager, return_names=True)
            collision = collision12 or collision13 or collision14 or collision15
            return collision, name_pairs
        else:
            distance12, name_pairs = self.robot1_collision_manager.min_distance_other(self.robot2_collision_manager, return_names=True)
            distance13, name_pairs = self.robot1_collision_manager.min_distance_other(self.robot3_collision_manager, return_names=True)
            distance14, name_pairs = self.robot1_collision_manager.min_distance_other(self.robot4_collision_manager, return_names=True)
            distance15, name_pairs = self.robot1_collision_manager.min_distance_other(self.robot5_collision_manager, return_names=True)
            return min(distance12, distance13, distance14, distance15), name_pairs
    ...
    def _self_collision_check(self, fk_dict, use_bb=False):
        ...
        for collision_manager in [self.robot1_collision_manager, self.robot2_collision_manager, self.robot3_collision_manager]:
...
if __name__ == '__main__':
    ...
    robot = robots2.ZonoArmRobot.load(os.path.join(os.getcwd(),'envs/arm_urdfs/ur5/five_ur5.urdf'), create_joint_occupancy=False)
    ...
    for i in range(100):
        qpos = env.qpos
        velocity_action = np.random.random(30) - 0.5
