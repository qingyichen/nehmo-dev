...
from envs.FiveArm_env import FiveArmEnv, FullStepRecorder, RobotStepViz
from agents.FiveArm_agent import FiveArmAgent
...
def parse_args():
    ...
    p.add_argument('--max_steps', type=int, default=300, help='The max number of steps allowed for a trial.')
    # rendering settings
...
def load_robot(opt):
    ...
    robot = robots2.ZonoArmRobot.load(os.path.join(os.getcwd(), f'envs/arm_urdfs/{robot_name}/five_{robot_name}.urdf'), create_joint_occupancy=False)
    return robot
...
def compute_plan(planner, qpos, qgoal, step_time=0.1, safe_time=0.3, timestep_discretization=10, buffer=0.0, planner_mode='hji'):
    ...
    env_to_qpos_mapping = np.argsort(qpos_to_env_mapping)

    env_qpos = qpos
    qpos = env_qpos[env_to_qpos_mapping]
    qgoal = qgoal[env_to_qpos_mapping]
    
    num_links = qpos.shape[0] // 5
    arm1_state = qpos[:num_links]
    arm2_state = qpos[num_links:num_links*2]
    arm3_state = qpos[num_links*2:num_links*3]
    arm1_goal = qgoal[:num_links]
    arm2_goal = qgoal[num_links:num_links*2]
    arm3_goal = qgoal[num_links*2:num_links*3]
    arm4_state = qpos[num_links*3:num_links*4]
    arm4_goal = qgoal[num_links*3:num_links*4]
    arm5_state = qpos[num_links*4:num_links*5]
    arm5_goal = qgoal[num_links*4:num_links*5]
    
    t0 = time.time()
    arm1_action = planner.plan(agent_state=arm1_state,
                            other_agent_state=np.stack([arm2_state, arm3_state, arm4_state, arm5_state], axis=0),
    ...
    arm2_action = planner.plan(agent_state=arm2_state,
                            ...
                            planner_mode=planner_mode).cpu().numpy()
    t2 = time.time()
    arm1_state_equiv = arm1_state.copy()
    arm1_state_equiv[0] = wrap_joint(np.pi + arm1_state_equiv[0])
    arm3_action = planner.plan(agent_state=arm3_state,
                            other_agent_state=arm1_state_equiv,
                            goal_state=arm3_goal,
                            step_time=step_time, 
                            safe_time=safe_time,
                            buffer=buffer,
                            planner_mode=planner_mode).cpu().numpy()
    ...
    arm1_state_equiv = arm1_state.copy()
    arm1_state_equiv[0] = wrap_joint(-np.pi/2 + arm1_state_equiv[0])
    arm4_action = planner.plan(agent_state=arm4_state,
                            other_agent_state=arm1_state_equiv,
                            goal_state=arm4_goal,
                            step_time=step_time, 
                            safe_time=safe_time,
                            buffer=buffer,
                            planner_mode=planner_mode).cpu().numpy()
    ...
    arm1_state_equiv = arm1_state.copy()
    arm1_state_equiv[0] = wrap_joint(+np.pi/2 + arm1_state_equiv[0])
    arm5_action = planner.plan(agent_state=arm5_state,
                            other_agent_state=arm1_state_equiv,
                            goal_state=arm5_goal,
                            step_time=step_time, 
                            safe_time=safe_time,
                            buffer=buffer,
                            planner_mode=planner_mode).cpu().numpy()
    ...
    velocity_action = np.concatenate((arm1_action, arm2_action, arm3_action, arm4_action, arm5_action))
...
if __name__ == '__main__':
    ...
    if opt.save_stats:
        ...
        if opt.save_traj:
            stats_folder = f'planning_traj/five{opt.robot}'
        else:
            stats_folder = f'planning_results/five{opt.robot}'
