...
from envs.TripleArm_env import TripleArmEnv, FullStepRecorder, RobotStepViz
from agents.TripleArm_agent import TripleArmAgent
...
def parse_args():
    ...
    p.add_argument('--benchmark', type=int, default=1) # 0 or 1
    # rendering settings
...
def load_robot(opt):
    ...
    robot = robots2.ZonoArmRobot.load(os.path.join(os.getcwd(), f'envs/arm_urdfs/{robot_name}/triple_{robot_name}.urdf'), create_joint_occupancy=False)
    return robot
...
def compute_plan(planner, qpos, qgoal, step_time=0.1, safe_time=0.3, timestep_discretization=10, buffer=0.0, planner_mode='hji'):
    ...
    qpos_to_env_mapping = np.array([0, 6, 12, 7, 13, 1, 2, 8, 14, 9, 3, 15, 10, 4, 16, 5, 11, 17], dtype=np.int32)
    env_to_qpos_mapping = np.array([0, 5, 6, 10, 13, 15, 1, 3, 7, 9, 12, 16, 2, 4, 8, 11, 14, 17], dtype=np.int32)

    env_qpos = qpos
    qpos = env_qpos[env_to_qpos_mapping]
    qgoal = qgoal[env_to_qpos_mapping]
    
    num_links = qpos.shape[0] // 3
    ...
    arm1_action = planner.plan(agent_state=arm1_state,
                            other_agent_state=np.stack([arm2_state, arm3_state], axis=0),
    ...
    velocity_action = np.concatenate((arm1_action, arm2_action, arm3_action))
...
if __name__ == '__main__':
    ...
    if video:
        ...
        video_folder = f'planning_videos/benchmark{opt.benchmark}_{opt.robot}/{planner_name}'
        if not os.path.exists(video_folder):
            os.makedirs(video_folder)
    if blender:
        blender_folder = f'planning_blenders/benchmark{opt.benchmark}_{opt.robot}/{planner_name}'
        if not os.path.exists(blender_folder):
            os.makedirs(blender_folder)
    
    # planning preparations
    robot = load_robot(opt=opt)
    planner = TripleArmAgent(
    ...
    for i_trial in tqdm(trials):
        planned_trajectoy = []
        env = TripleArmEnv(robot=robot.urdf,
    ...
    if opt.save_stats:
        ...
        if opt.save_traj:
            stats_folder = f'planning_traj/triple{opt.robot}'
        else:
            stats_folder = f'planning_results/triple{opt.robot}'
